...
//
// RaiseStatusIndex, Unwind, Exception are new to Vista.
//
// RaiseStatusIndex is set to the index into RtlpFunctionAddressTable for the
// raise exception routine for the processor mode.  In the user mode version
// of RtlpUnwindHistoryTable, this is RtlRaiseException.  In the kernel mode
// version of RtlpUnwindHistoryTable, this is RtlRaiseStatus.
//
// Note that RaiseStatusIndex is never used in a user-supplied unwind history
// table; it is only used by the global RtlpFunctionAddressTable that lives in
// ntoskrnl or ntdll.
//
// Unwind and Exception are hints to the function lookup logic.  These are
// typically supplied with a user-supplied unwind history table for use with a
// one-shot lookup operation (i.e. where RtlLookupFunctionEntry is only going
// to be called once, and not in a loop).  This may initially seem strange, as
// the documented purpose of the UNWIND_HISTORY_TABLE argument is as a cache
// that is used to speed up repeated address lookups for exception or unwind
// dispatching.  However, these two flags enable undocumented behaviors on
// Windows Vista (or later).
//
// Setting Unwind to TRUE causes an initial lookup against a flat array of
// function addresses stored in RtlpFunctionAddressTable (part of ntoskrnl or
// ntdll).
//
